...
class HDREquirectangularTextureLoader {

  constructor(renderer) {
    this.manager = THREE.DefaultLoadingManager;
    this.hdrLoader = new RGBELoader();
    this.equiToCube = new EquirectangularToCubemap( renderer );
  }

  load( type, url, onLoad, onProgress = undefined, onError = undefined) {
    ...
    loader.load( url,  ( buffer ) => {
      ...
      if ( undefined !== texData.image ) {
        ...
        texture.image = dataTexture;

      }

      texture.needsUpdate = true;
      console.log('Origin HDR texture', texture);
      cubeMapTexture = this.equiToCube.convert( texture, 2048 );
      cubeMapTexture.needsUpdate = true;
      console.log('Cubemap texture', cubeMapTexture);

      if (onLoad) {
        onLoad(cubeMapTexture);
      }
...
export default HDREquirectangularTextureLoader;